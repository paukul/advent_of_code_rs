...
pub fn generator(input: &str) -> Vec<(NaiveDateTime, &str)> {
  let mut lines_with_time: Vec<(NaiveDateTime, &str)> = input.lines().map(|line| {
    NaiveDateTime::parse_from_str(&line[1..17], FORMAT).map(|time| (time, line))
  }).flatten().collect();

  lines_with_time.sort_by(|(time_a, _), (time_b, _)| {
    time_a.cmp(time_b)
  });
...
pub mod test {
  ...
  #[test]
  fn test_generator() {
    let input = vec!["[1518-09-29 00:35] falls asleep",
                     "[1518-09-25 00:28] falls asleep",
                     "[1518-10-14 00:00] Guard #2927 begins shift",
                     "[1518-07-20 00:53] wakes up"].join("\n");
